      ...
      <section class="section card pipeline-section">
        <div class="one-click-actions">
          ...
          <el-button
            :loading="pipelineRunning && !pipelinePaused"
            :disabled="!currentEpisodeId || pipelineRunning"
            title="只补齐缺失的角色、场景、道具和分镜；已有角色描述不会重写，不生成图片与视频"
          ...
          <el-button
            type="success"
            plain
            :loading="continuityAuditRunning"
            :disabled="!currentEpisodeId || pipelineRunning || continuityAuditRunning"
            title="不调用模型、不产生费用：按镜序锁定人物服装，找出参考图与剧情服装冲突，并提示需要局部重生的镜头"
            @click="onAuditCharacterContinuity"
          >
            人物连戏校对
          </el-button>
...
const continuityAuditRunning = ref(false)

function continuityAuditMessage(result) {
  const summary = result?.summary || {}
  const lines = [
    `已检查 ${result?.checked_storyboards || 0} 个分镜，并保存逐镜服装锁。`,
    `参考图服装冲突：${summary.reference_conflicts || 0} 处；无换装动作的突变：${summary.unexpected_changes || 0} 处；自动继承：${summary.inherited_locks || 0} 处。`,
  ]
  const regen = result?.regenerate_storyboard_numbers || []
  if (regen.length) {
    lines.push(`已有图片需要局部重生：第 ${regen.join('、')} 镜。`)
  } else {
    lines.push('现有图片无需因本次校对重生。')
  }
  const regenVideos = result?.regenerate_video_storyboard_numbers || []
  if (regenVideos.length) {
    lines.push(`已有视频需要随图片重生：第 ${regenVideos.join('、')} 镜。`)
  } else {
    lines.push('现有视频无需因本次校对重生。')
  }
  lines.push('校对本身不会重新生成图片或视频，也不会产生费用。')
  const visibleIssues = (result?.issues || []).filter((issue) => issue.severity !== 'info').slice(0, 6)
  for (const issue of visibleIssues) lines.push(`• ${issue.message}`)
  return lines.join('\n')
}

function screenCompositionAuditMessage(result) {
  const shots = result?.affected_storyboard_numbers || []
  const regenImages = result?.regenerate_storyboard_numbers || []
  const regenVideos = result?.regenerate_video_storyboard_numbers || []
  if (!shots.length && !regenImages.length && !regenVideos.length) {
    return '人物表情与剧情屏幕信息的构图无需修复。'
  }
  const lines = []
  if (shots.length) {
    lines.push(`屏幕信息双层构图已写入：第 ${shots.join('、')} 镜（人物主画面＋侧边手机 UI 浮层）。`)
  }
  if (regenImages.length) lines.push(`旧分镜图需局部重生：第 ${regenImages.join('、')} 镜。`)
  if (regenVideos.length) lines.push(`旧分镜视频需局部重生：第 ${regenVideos.join('、')} 镜。`)
  return lines.join('\n')
}

async function runCharacterContinuityAudit({ showResult = false } = {}) {
  if (!currentEpisodeId.value) return null
  continuityAuditRunning.value = true
  try {
    const result = await storyboardsAPI.auditContinuity(currentEpisodeId.value, { apply: true })
    await loadDrama()
    if (showResult) {
      await ElMessageBox.alert(continuityAuditMessage(result), '人物连戏校对完成', {
        confirmButtonText: '知道了',
        type: (result?.regenerate_storyboard_numbers || []).length ? 'warning' : 'success',
      })
    }
    return result
  } finally {
    continuityAuditRunning.value = false
  }
}

async function onAuditCharacterContinuity() {
  try {
    await runCharacterContinuityAudit({ showResult: true })
  } catch (e) {
    ElMessage.error(e.message || '人物连戏校对失败')
  }
}
...
async function saveProjectSettings(includeGenerationStyle = false) {
  ...
  try {
    ...
    if (includeGenerationStyle) {
      await loadDrama()
      ElMessage.success('画风已更新；旧角色提示词已刷新，现有图片保留')
    }
  } catch (e) {
...
async function runOneClickPipeline(textOnly = false) {
  ...
  const continuityForceRegenerateImageIds = new Set()
  const continuityForceRegenerateVideoIds = new Set()
  ...
  try {
    ...
    if (boards.length === 0) {
      ...
      setPipelineStep(4, `已有 ${boards.length} 条分镜脚本，跳过文本重建（仍会执行人物连戏校对）`)
    }
    ...
    // 分镜文本完成后先做零费用质量校对：人物连戏 + 剧情屏幕信息构图。
    try {
      setPipelineStep(4, '正在执行人物连戏与屏幕构图校对（零费用）...')
      const continuityResult = await storyboardsAPI.auditContinuity(episodeId, { apply: true })
      const screenCompositionResult = await storyboardsAPI.auditScreenComposition(episodeId, { apply: true })
      await loadDrama()
      boards = store.storyboards || []
      const regenIds = [...new Set([
        ...(continuityResult?.regenerate_storyboard_ids || []),
        ...(screenCompositionResult?.regenerate_storyboard_ids || []),
      ])]
      const regenVideoIds = [...new Set([
        ...(continuityResult?.regenerate_video_storyboard_ids || []),
        ...(screenCompositionResult?.regenerate_video_storyboard_ids || []),
      ])]
      const repairShotNumbers = [...new Set([
        ...(continuityResult?.regenerate_storyboard_numbers || []),
        ...(continuityResult?.regenerate_video_storyboard_numbers || []),
        ...(screenCompositionResult?.regenerate_storyboard_numbers || []),
        ...(screenCompositionResult?.regenerate_video_storyboard_numbers || []),
      ])]
      if (!textOnly && (regenIds.length || regenVideoIds.length)) {
        try {
          await ElMessageBox.confirm(
            `${continuityAuditMessage(continuityResult)}\n\n${screenCompositionAuditMessage(screenCompositionResult)}\n\n是否只重新生成这些受影响镜头的图片和视频？确认后才会产生相应模型费用。`,
            '发现需要修复的画面一致性问题',
            {
              confirmButtonText: '局部重生并继续',
              cancelButtonText: '停止流程，手动处理',
              type: 'warning',
              closeOnClickModal: false,
              closeOnPressEscape: false,
            }
          )
          regenIds.forEach((id) => continuityForceRegenerateImageIds.add(Number(id)))
          regenVideoIds.forEach((id) => continuityForceRegenerateVideoIds.add(Number(id)))
          setPipelineStep(
            4,
            repairShotNumbers.length
              ? `连戏校对完成：后续将局部重生第 ${repairShotNumbers.join('、')} 镜`
              : '连戏校对完成：后续将局部重生受影响镜头'
          )
        } catch (_) {
          const blockedMessage = '检测到画面一致性问题，已停止一键流程；请手动处理后重试，或重新运行并选择“局部重生并继续”。'
          addPipelineError('画面一致性校对', blockedMessage)
          ElMessage.warning(blockedMessage)
          return
        }
      } else if (repairShotNumbers.length) {
        setPipelineStep(4, `画面一致性校对完成：第 ${repairShotNumbers.join('、')} 镜需要局部重生`)
      } else {
        setPipelineStep(4, '分镜脚本、人物连戏与屏幕构图校对已通过')
      }
    } catch (e) {
      addPipelineError('画面一致性校对', e.message || String(e))
      return
    }
    ...
    // 步骤 8：生成分镜图
    {
      await loadStoryboardMedia()
      boards = store.storyboards || []
      const boardsWithoutImg = boards.filter((sb) => !hasSbImage(sb) || continuityForceRegenerateImageIds.has(Number(sb.id)))
      const forcedImageShotNumbers = boardsWithoutImg
        .filter((sb) => continuityForceRegenerateImageIds.has(Number(sb.id)))
        .map((sb) => sb.storyboard_number ?? sb.id)
      const concurrency = pipelineConcurrency.value
      setPipelineStep(
        8,
        forcedImageShotNumbers.length
          ? `人物连戏局部重生：第 ${forcedImageShotNumbers.join('、')} 镜（本轮共 ${boardsWithoutImg.length} 个，并发 ${concurrency}）...`
          : `生成分镜图（${boardsWithoutImg.length} 个，并发 ${concurrency}）...`
      )
    ...
    // 步骤 9：生成分镜视频
    {
      ...
      const boards2 = (store.storyboards || []).filter((sb) => {
        const vidList = sbVideos.value[sb.id] || []
        const forceRegenerateVideo = continuityForceRegenerateVideoIds.has(Number(sb.id))
        if (!forceRegenerateVideo && vidList.some((v) => v.status === 'completed' && recordHasPlayableVideoUrl(v))) return false
        if (isSbUniversalMode(sb.id)) {
          if (!sbCanSubmitVideo(sb)) return false
          return collectSbOmniReferenceAbsoluteUrls(sb).length > 0
        }
      ...
      const forcedVideoShotNumbers = boards2
        .filter((sb) => continuityForceRegenerateVideoIds.has(Number(sb.id)))
        .map((sb) => sb.storyboard_number ?? sb.id)
      const concurrency = pipelineVideoConcurrency.value
      setPipelineStep(
        9,
        forcedVideoShotNumbers.length
          ? `人物连戏局部重生视频：第 ${forcedVideoShotNumbers.join('、')} 镜（本轮共 ${boards2.length} 个，并发 ${concurrency}）...`
          : `生成分镜视频（${boards2.length} 个，并发 ${concurrency}）...`
      )
      const { paused } = await runConcurrently(boards2, concurrency, async (sb) => {
        ...
        try {
          ...
          const ok = await pipelineWithRetry(stepName, async () => {
            if (continuityForceRegenerateVideoIds.has(Number(sb.id))) {
              await storyboardsAPI.update(sb.id, { video_url: null })
              if (sbSelectedVideoId.value[sb.id] != null) {
                const next = { ...sbSelectedVideoId.value }
                delete next[sb.id]
                sbSelectedVideoId.value = next
