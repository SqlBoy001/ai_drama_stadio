...
export const storyboardsAPI = {
  ...
  /** 无模型调用：逐镜校对人物服装状态并写入连戏快照 */
  auditContinuity(episodeId, data = { apply: true }) {
    return request.post(`/episodes/${episodeId}/continuity-audit`, data)
  },
  /** 无模型调用：修复人物表情与剧情屏幕内容同镜展示时的物理构图冲突 */
  auditScreenComposition(episodeId, data = { apply: true }) {
    return request.post(`/episodes/${episodeId}/screen-composition-audit`, data)
  },
  upscale(id) {
