...
| `Doubao-Seedance-2.0-mini` | `doubao-seedance-2-0-mini-260615` | 低成本草稿、空镜和简单运动 |
| `Doubao-Seedance-2.5` | `doubao-seedance-2-5-260628` | 复杂动作、多人互动和重点高潮镜头 |

AI 工作台在同一条视频配置包含上述型号时，会按镜头自动选择：空镜用 Mini、简单单人动作用 Fast、单人对白用 1.5 Pro、多人连戏用 2.0、复杂互动与高潮用 2.5。未配置某一档时会自动回退到可用型号，不会阻断生产。
