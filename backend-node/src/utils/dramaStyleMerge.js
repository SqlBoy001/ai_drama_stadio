...
/** 显式画风参数可以是预设 key 或完整描述；预设 key 必须展开后再覆盖。 */
function applyStyleOverrideToCfg(cfg, styleOverride) {
  const value = (styleOverride || '').toString().trim();
  if (!value || value === 'custom') return cfg;
  const preset = resolveStylePreset(value);
  const zh = preset?.zh || value;
  const en = preset?.en || value;
  return {
    ...cfg,
    style: {
      ...(cfg?.style || {}),
