...
const characterContinuityService = require('../services/characterContinuityService');
const screenContentCompositionService = require('../services/screenContentCompositionService');
...
function routes(db, log) {
  return {
    ...
    // 人物连戏校对：无模型调用，按镜序锁定服装状态并报告需局部重生的已有图片。
    auditContinuity: (req, res) => {
      try {
        const episodeId = Number(req.params.episode_id);
        if (!episodeId) return response.badRequest(res, 'episode_id 必填');
        const result = characterContinuityService.auditEpisodeContinuity(db, episodeId, {
          apply: req.body?.apply !== false,
        });
        log.info('[人物连戏校对] 完成', {
          episode_id: episodeId,
          affected: result.affected_storyboard_ids.length,
          regenerate: result.regenerate_storyboard_ids.length,
          issues: result.issues.length,
        });
        response.success(res, result);
      } catch (err) {
        log.error('[人物连戏校对] 失败', { error: err.message, episode_id: req.params.episode_id });
        response.badRequest(res, err.message || '人物连戏校对失败');
      }
    },

    // 屏幕信息构图校对：无模型调用，修复“正面人物 + 朝向人物的手机屏幕”物理冲突。
    auditScreenComposition: (req, res) => {
      try {
        const episodeId = Number(req.params.episode_id);
        if (!episodeId) return response.badRequest(res, 'episode_id 必填');
        const result = screenContentCompositionService.auditEpisodeScreenComposition(db, episodeId, {
          apply: req.body?.apply !== false,
        });
        log.info('[屏幕信息构图校对] 完成', {
          episode_id: episodeId,
          affected: result.affected_storyboard_ids.length,
          regenerate_images: result.regenerate_storyboard_ids.length,
          regenerate_videos: result.regenerate_video_storyboard_ids.length,
        });
        response.success(res, result);
      } catch (err) {
        log.error('[屏幕信息构图校对] 失败', { error: err.message, episode_id: req.params.episode_id });
        response.badRequest(res, err.message || '屏幕信息构图校对失败');
      }
    },
    ...
    polishPrompt: async (req, res) => {
      try {
        ...
        let currentContinuityState = null;
        if (sb.episode_id != null && sb.storyboard_number != null) {
          try {
            characterContinuityService.auditEpisodeContinuity(db, sb.episode_id, { apply: true });
            const currentStateRow = db.prepare(
              'SELECT continuity_snapshot FROM storyboards WHERE id = ? AND deleted_at IS NULL'
            ).get(sbId);
            if (currentStateRow?.continuity_snapshot) currentContinuityState = JSON.parse(currentStateRow.continuity_snapshot);
          } catch (_) {}
        ...
        const userPromptLines = [
          ...
          currentContinuityState ? `CURRENT_CONTINUITY_STATE: ${JSON.stringify(currentContinuityState)}` : null,
