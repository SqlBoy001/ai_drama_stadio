...
function runFfmpegConcat(localPaths, outputPath, log, targetDuration) {
  ...
  try {
    ...
    const args = [
      ...
      '-safe', '0',
      '-i', listFile,
      // 供应商片段和本地兜底片段可能使用不同 fps/time_base。
      // concat copy 会累积时间戳漂移，因此统一重编码成恒定 25fps。
      '-an',
      '-c:v', 'libx264',
      '-preset', 'fast',
      '-crf', '20',
      ...
      '-r', '25',
      '-vsync', 'cfr',
      ...(Number(targetDuration) > 0 ? ['-t', String(Number(targetDuration))] : []),
      '-y',
      outputPath,
    ];
...
async function processVideoMerge(db, log, mergeId, baseUrl) {
  ...
  if (localPaths.length > 0 && ffmpegAvailable && localPaths.length <= 100) {
    ...
    const ok = runFfmpegConcat(localPaths, outputPath, log, totalDuration);
