...
function buildStoryboardContext(cfg, sb, scene, characterNames) {
  ...
  if (sb.continuity_snapshot) {
    try {
      const continuityLock = characterContinuityService.buildContinuityLock(JSON.parse(sb.continuity_snapshot));
      if (continuityLock) parts.unshift(continuityLock);
    } catch (_) {}
  }
  if (isEn) {
...
async function processFramePromptGeneration(db, log, taskId, storyboardId, frameType, panelCount, model) {
  ...
  try {
    const seed = db.prepare('SELECT episode_id FROM storyboards WHERE id = ? AND deleted_at IS NULL').get(Number(storyboardId));
    if (seed?.episode_id) {
      characterContinuityService.auditEpisodeContinuity(db, seed.episode_id, { apply: true });
    }
  } catch (continuityErr) {
    log.warn('[帧提示词] 人物服装连戏校对失败，继续生成', { storyboard_id: storyboardId, error: continuityErr.message });
  }
