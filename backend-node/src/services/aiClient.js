...
// 这些任务必须把有限输出额度留给最终 JSON/正文。DeepSeek 思考模式会先消耗
// reasoning_content，短额度下可能正常结束却没有任何 content。
const STRUCTURED_OUTPUT_SCENES = new Set([
  'story_generation',
  'role_extraction',
  'scene_extraction',
  'prop_extraction',
...
function applyRequestThinkingPolicy(config, body, options = {}) {
  let next = applyDeepSeekChatOptions(config, body);
  if (!isDeepSeekOfficialConfig(config)) return next;
  const explicit = options.deepseek_thinking;
  const policy = explicit || (STRUCTURED_OUTPUT_SCENES.has(options.scene_key) ? 'disabled' : null);
  if (!policy) return next;
  const normalized = String(policy).toLowerCase() === 'enabled' ? 'enabled' : 'disabled';
  next = { ...next, thinking: { type: normalized } };
...
async function generateText(db, log, serviceType, userPrompt, systemPrompt, options = {}) {
  ...
  body = applyRequestThinkingPolicy(config, body, { ...options, scene_key });
