...
async function testConnection(opts) {
  ...
  if (treatAsImage) {
    ...
    if (!res.ok) {
      ...
      const isCapabilityErr = lmsg.includes('modelnotopen') || lmsg.includes('has not activated the model')
        || lmsg.includes('not supported when using codex') || lmsg.includes('model is not supported');
      if (isCapabilityErr) throw new Error(`模型不可用: ${msg || res.status}`);
