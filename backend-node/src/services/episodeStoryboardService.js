...
function generateImagePrompt(sb, style) {
  ...
  parts.push('首帧静止画面');
  return screenContentCompositionService.applyScreenContentComposition(parts.join('，'), sb, 'image');
}
...
function generateVideoPrompt(sb, style, videoRatio) {
  ...
  const prompt = parts.length ? parts.join('。') : '视频场景';
  return screenContentCompositionService.applyScreenContentComposition(prompt, sb, 'video');
}
