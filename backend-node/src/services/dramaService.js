...
function replaceKnownStyleText(value, previousMetadata, nextMetadata) {
  let text = value == null ? value : String(value);
  if (!text) return text;
  const pairs = [
    [previousMetadata.style_prompt_zh, nextMetadata.style_prompt_zh],
    [previousMetadata.style_prompt_en, nextMetadata.style_prompt_en],
  ];
  for (const [before, after] of pairs) {
    const from = (before || '').toString().trim();
    const to = (after || '').toString().trim();
...
function saveOutline(db, log, dramaId, req) {
  ...
  const previousStyleSignature = JSON.stringify([
    drama.style || '',
    existingMetadata.style_prompt_zh || '',
    existingMetadata.style_prompt_en || '',
  ]);
  ...
  const nextStyle = req.style !== undefined ? req.style : drama.style;
  const nextStyleSignature = JSON.stringify([
    nextStyle || '',
    mergedMetadata.style_prompt_zh || '',
    mergedMetadata.style_prompt_en || '',
  ]);
  const hasExplicitStyleUpdate = req.style !== undefined ||
    Object.prototype.hasOwnProperty.call(newMetadata, 'style_prompt_zh') ||
    Object.prototype.hasOwnProperty.call(newMetadata, 'style_prompt_en');
  const styleChanged = hasExplicitStyleUpdate && previousStyleSignature !== nextStyleSignature;
  ...
  if (styleChanged) {
    const rows = db.prepare(
      'SELECT id, appearance FROM characters WHERE drama_id = ? AND deleted_at IS NULL'
    ).all(Number(dramaId));
    const updateCharacter = db.prepare(
      'UPDATE characters SET appearance = ?, polished_prompt = NULL, updated_at = ? WHERE id = ?'
    );
    for (const row of rows) {
      updateCharacter.run(stripRenderStyleFromAppearance(row.appearance), now, row.id);
    }
    const sceneRows = db.prepare(
      'SELECT id, prompt FROM scenes WHERE drama_id = ? AND deleted_at IS NULL'
    ).all(Number(dramaId));
    const updateScene = db.prepare(
      'UPDATE scenes SET prompt = ?, polished_prompt = NULL, polished_prompt_single = NULL, updated_at = ? WHERE id = ?'
    );
    for (const row of sceneRows) {
      updateScene.run(replaceKnownStyleText(row.prompt, existingMetadata, mergedMetadata), now, row.id);
    }
    const propRows = db.prepare(
      'SELECT id, prompt FROM props WHERE drama_id = ? AND deleted_at IS NULL'
    ).all(Number(dramaId));
    const updateProp = db.prepare('UPDATE props SET prompt = ?, updated_at = ? WHERE id = ?');
    for (const row of propRows) {
      updateProp.run(replaceKnownStyleText(row.prompt, existingMetadata, mergedMetadata), now, row.id);
    }
    const storyboardRows = db.prepare(
      `SELECT sb.id, sb.image_prompt, sb.video_prompt
       FROM storyboards sb
       JOIN episodes ep ON ep.id = sb.episode_id
       WHERE ep.drama_id = ? AND sb.deleted_at IS NULL AND ep.deleted_at IS NULL`
    ).all(Number(dramaId));
    const updateStoryboard = db.prepare(
      'UPDATE storyboards SET image_prompt = ?, video_prompt = ?, polished_prompt = NULL, updated_at = ? WHERE id = ?'
    );
    for (const row of storyboardRows) {
      updateStoryboard.run(
        replaceKnownStyleText(row.image_prompt, existingMetadata, mergedMetadata),
        replaceKnownStyleText(row.video_prompt, existingMetadata, mergedMetadata),
        now,
        row.id,
      );
    }
    log.info('Character style-dependent prompts invalidated', {
      drama_id: dramaId,
      character_count: rows.length,
      scene_count: sceneRows.length,
      prop_count: propRows.length,
...
function getVideoUrlForStoryboard(db, storyboardId, baseUrl) {
  ...
  // storyboards.local_path 存的是关键帧图片，不能作为视频路径使用。
  const sb = db.prepare('SELECT video_url, updated_at FROM storyboards WHERE id = ? AND deleted_at IS NULL').get(storyboardId);
  ...
  // videoService 会在同一时刻更新分镜与生成记录；时间相等时优先使用
  // video_generations.local_path，避免重新下载会过期的远程签名 URL。
  if (vgTime >= sbTime) {
    return vgUrl;
  }
