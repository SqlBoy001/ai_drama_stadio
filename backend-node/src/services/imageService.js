...
const screenContentCompositionService = require('./screenContentCompositionService');
...
async function processImageGeneration(db, log, imageGenId) {
  ...
  try {
      ...
      try {
        ...
        if (anyTextConfig) {
          ...
          if (sbDetail?.episode_id != null && sbDetail?.storyboard_number != null) {
            try {
              characterContinuityService.auditEpisodeContinuity(db, sbDetail.episode_id, { apply: true });
              const currentStateRow = db.prepare(
                'SELECT continuity_snapshot FROM storyboards WHERE id = ? AND deleted_at IS NULL'
              ).get(Number(row.storyboard_id));
              if (currentStateRow?.continuity_snapshot) {
                currentContinuityState = JSON.parse(currentStateRow.continuity_snapshot);
              }
    ...
    // ── Step 3.7: 注入人物服装连戏硬锁 ────────────────────────────────
    // 角色参考图可能是“默认造型”，剧情分镜则可能要求另一套服装。
    // 当前镜头的连续状态始终高于参考图服装；参考图仅保留身份锚点用途。
    if (isSingleStoryboard && row.storyboard_id && finalPrompt) {
      try {
        finalPrompt = characterContinuityService.applyStoryboardContinuityLock(
          db,
          Number(row.storyboard_id),
          finalPrompt
        );
        reference_context_note = characterContinuityService.annotateCharacterReferenceLabels(reference_context_note);
        db.prepare('UPDATE image_generations SET prompt = ?, updated_at = ? WHERE id = ?').run(
          finalPrompt,
        ...
        log.warn('[图生] 人物服装连戏锁注入失败，继续使用现有提示词', {
          id: imageGenId,
          storyboard_id: row.storyboard_id,
          error: continuityErr.message,
        });
      }
    }

    // ── Step 3.75: 剧情屏幕信息采用“人物反应 + 侧边 UI 浮层”双信息层构图 ──
    // 同一正面机位无法同时看清人物表情和朝向人物的实体手机屏幕；强制避免背壳发光。
    if (isSingleStoryboard && row.storyboard_id && finalPrompt) {
      try {
        finalPrompt = screenContentCompositionService.applyStoryboardScreenComposition(
          db,
          Number(row.storyboard_id),
          finalPrompt,
          'image'
      ...
      } catch (screenCompositionErr) {
        log.warn('[图生] 手机信息双层构图锁注入失败，继续使用现有提示词', {
          id: imageGenId,
          storyboard_id: row.storyboard_id,
          error: screenCompositionErr.message,
        });
      }
    }
