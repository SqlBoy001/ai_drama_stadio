...
async function finalizeSuccessfulVideo(db, log, videoGenId, row, rowForAspect, videoUrl, logLabel) {
  ...
  if (row.storyboard_id) {
    try {
      // storyboards.local_path 保存关键帧图片；视频本地路径只属于 video_generations。
...
async function processVideoGeneration(db, log, videoGenId) {
  ...
  try {
    ...
    const resolvedModel = videoClient.resolveConfiguredVideoModel(config, row.model);
    if (resolvedModel && resolvedModel !== row.model) {
      const requestedModel = row.model;
      db.prepare('UPDATE video_generations SET model = ?, updated_at = ? WHERE id = ?')
        .run(resolvedModel, new Date().toISOString(), videoGenId);
      row.model = resolvedModel;
      log.info('Video model resolved to active configuration', {
    ...
    let promptForApi = row.prompt;
    if (row.storyboard_id && promptForApi) {
      try {
        promptForApi = characterContinuityService.applyStoryboardContinuityLock(
          db,
          Number(row.storyboard_id),
          promptForApi
        );
        if (promptForApi !== row.prompt) {
          db.prepare('UPDATE video_generations SET prompt = ?, updated_at = ? WHERE id = ?')
            .run(promptForApi, new Date().toISOString(), videoGenId);
        }
      } catch (continuityErr) {
        log.warn('[视频] 人物服装连戏锁注入失败，继续使用现有提示词', {
          video_gen_id: videoGenId,
          storyboard_id: row.storyboard_id,
    ...
    if (row.storyboard_id && promptForApi) {
      try {
        promptForApi = screenContentCompositionService.applyStoryboardScreenComposition(
          db,
          Number(row.storyboard_id),
          promptForApi,
          'video'
        ...
        log.warn('[视频] 手机信息双层构图锁注入失败，继续使用现有提示词', {
          video_gen_id: videoGenId,
          storyboard_id: row.storyboard_id,
