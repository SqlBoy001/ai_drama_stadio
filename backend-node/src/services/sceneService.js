...
const { applyStyleOverrideToCfg, mergeCfgStyleWithDrama } = require('../utils/dramaStyleMerge');

function scenePromptMatchesStyle(prompt, cfg) {
...
async function generateSceneFourViewImage(db, log, cfg, sceneId, modelName, style) {
  ...
  if (scenePromptMatchesStyle(sceneRow.polished_prompt, mergedCfg)) {
...
async function generateSceneSingleImage(db, log, cfg, sceneId, modelName, style) {
  ...
  if (scenePromptMatchesStyle(sceneRow.polished_prompt_single, mergedCfg)) {
