...
2. For each character, extract:
   ...
   - appearance: Detailed physical identity facts for AI image generation (gender, age, body type, facial features, hairstyle, clothing — NO scene/background info and NO art style, render style, medium, or phrases such as "fits the XXX style")
...
- **Downstream Render Style**: ${style} (context only; NEVER copy this style name or wording into appearance/description; the system injects it later during image generation)
...
Each element is a character object containing the above fields.`;
  ...
  if (_charOverride) {
    return _charOverride + `\n- **字段隔离**：appearance 只写人物身份事实，不要写入画风名称、渲染方式或“符合/贴合某某风格”等评价句；画风由系统在生图时统一注入。\n- **后续渲染风格（仅供理解，不得抄入字段）**：${style}\n- **图片比例**：${imageRatio}\n输出格式：\n**重要：必须只返回纯JSON数组，不要包含任何markdown代码块、说明文字或其他内容。直接以 [ 开头，以 ] 结尾。**\n每个元素是一个角色对象，包含上述字段。`;
  }
...
2. 对每个角色，提取以下信息（全部用中文填写）：
   ...
   - appearance: 外貌与身份事实（中文，100-200字，包含性别、年龄、体型、面部特征、发型、具体服装等，不含任何场景或环境信息；不要写入画风名称、渲染方式或“符合/贴合某某风格”等评价句）
   - description: 背景故事和角色关系（中文，50-100字）
3. 主要角色外貌要详细，次要角色可简化
- **字段隔离**：appearance 只写人物身份事实，画风由系统在生图时统一注入
- **后续渲染风格（仅供理解，不得抄入字段）**：${style}
- **图片比例**：${imageRatio}
输出格式：
**重要：必须只返回纯JSON数组，不要包含任何markdown代码块、说明文字或其他内容。直接以 [ 开头，以 ] 结尾。**
...
7. If the input contains a WARDROBE CONTINUITY LOCK, copy each character's locked outfit exactly into the prompt. It overrides clothing visible in reference images; reference images are identity-only in that case
...
7. 【服装连戏铁律】如 CONTEXT 含「人物服装连戏最高优先级」，必须把每个角色锁定的服装逐字写入 prompt；该服装锁高于参考图服装，参考图此时只锁身份
...
6. If the input contains a WARDROBE CONTINUITY LOCK, copy each character's locked outfit exactly into the prompt. It overrides clothing visible in reference images; reference images are identity-only in that case
...
7. 展示角色高潮状态下的肢体姿态和神情
8. 【服装连戏铁律】如 CONTEXT 含「人物服装连戏最高优先级」，必须把每个角色锁定的服装逐字写入 prompt；该服装锁高于参考图服装，参考图此时只锁身份
...
6. Include character appearance details if provided (ONLY fixed identity anchors from the provided CHARACTER VISUAL ANCHORS block. Copy exactly the traits listed there. NEVER hallucinate new hair style/color/length, face shape, expression details, or temperament not explicitly present in the anchor. If no detailed anchor is provided for a character, write only "Name (use appearance from reference image)" and add ZERO invented visual details)
6a. If the input contains a WARDROBE CONTINUITY LOCK, copy each character's locked outfit exactly into the prompt. It overrides clothing visible in reference images; reference images are identity-only in that case
...
7a. 【服装连戏铁律】如 CONTEXT 含「人物服装连戏最高优先级」，必须把每个角色锁定的服装逐字写入 prompt；该服装锁高于参考图服装，参考图此时只锁身份
...
4. Prompt requirements:
   ...
   - For suspense/horror stories, include one restrained, visible, story-relevant narrative anomaly derived from the script (for example an interrupted trail, impossible light direction, or a displaced object). Do not substitute generic cobwebs, blood stains, or fog for story evidence.
   - **Style Requirement**: ${s}
   - **Image Ratio**: ${imageRatio}
...
5. 悬疑/恐怖题材的每个场景必须包含一个克制、可见且服务剧情的**叙事异常点**，从剧本线索推导（如中断的痕迹、违反常理的光线方向、位置错乱的物件）；不得用蜘蛛网、血迹、雾气等廉价符号堆砌代替剧情证据
6. **风格要求**：${s}
   - **图片比例**：${imageRatio}
 ...
 */
function getStoryExpansionSystemPrompt(cfg, episodeCount, episodeDurationSeconds) {
  const n = Number(episodeCount) > 1 ? Number(episodeCount) : 1;
  const seconds = Math.max(15, Math.round(Number(episodeDurationSeconds) || 60));
  const minChars = Math.round(seconds * 8);
  const maxChars = Math.round(seconds * 12);
  const spokenChars = Math.round(seconds * 4);
  const jsonNote = `\n\n**输出格式（必须严格遵守）**：\n返回一个 JSON 数组，包含 ${n} 个对象，每个对象格式如下：\n[\n  {\n    "episode": 1,\n    "title": "第一集标题（5-10字，概括本集核心内容）",\n    "content": "本集剧本正文（约${minChars}-${maxChars}字）"\n  }\n]\n**必须只返回纯 JSON 数组，不要任何 markdown 代码块、说明文字。直接以 [ 开头，以 ] 结尾。**`;
  if (isEnglish(cfg)) {
    const minWords = Math.round(seconds * 2);
    const maxWords = Math.round(seconds * 3);
    const enNote = `\n\n**Output format (STRICTLY required)**:\nReturn a JSON array with ${n} object(s), each in this format:\n[\n  {\n    "episode": 1,\n    "title": "Episode title (5-15 words)",\n    "content": "Episode script body (~${minWords}-${maxWords} words)"\n  }\n]\n**Return ONLY the JSON array. No markdown, no explanation. Start directly with [ and end with ].**`;
...
3. Each episode must fit ${seconds} seconds. Keep spoken dialogue concise and make every action visually shootable.
4. Hook viewers in the first 3 seconds with danger, injustice, an impossible question, or a system-trigger event.
5. Give the protagonist a concrete goal and active choice, escalate external pressure, deliver one reveal or payoff, then end the final 3 seconds on an unresolved turn.
6. Every episode needs a complete mini-arc; multiple episodes must continue causally rather than repeat the premise.${enNote}`;
  }
...
3. 如有多集，剧情必须前后衔接——每集从上一集结尾处继续推进，不重复设定。
4. 人物必须有明确目标、主动选择和行动结果，避免只有气氛、旁白或设定说明。`;
  const durationContract = `\n\n【短剧节奏硬约束】
- 每集成片目标时长：${seconds} 秒；本时长约束优先于上文任何固定字数要求。
- 正文建议约 ${minChars}-${maxChars} 个中文字符，其中可朗读的对白与旁白合计不超过约 ${spokenChars} 字，其余用于精炼的动作、表情和因果描述。
- 前三秒必须出现危险、不公、异常谜题或系统触发之一，不能从日常铺垫慢慢开始。
- 中段让主角围绕一个具体目标主动行动，阻力和代价至少升级一次；必须兑现一个信息揭示、能力效果或情绪爽点。
- 最后三秒形成新的未解决危机、身份反转或选择困境；同时保证本集至少完成一个小闭环，不能无头无尾。
- 所有动作必须可视化、可表演；对白必须与动作和神态同步，删除重复解释和无效寒暄。`;
  return base + durationContract + jsonNote;
}
 ...
 */
function buildStoryExpansionUserPrompt(cfg, premise, style, type, episodeCount, episodeDurationSeconds) {
  const lang = isEnglish(cfg) ? 'en' : 'zh';
  const n = Number(episodeCount) > 1 ? Number(episodeCount) : 1;
  ...
  if (lang === 'en') {
    ...
    prompt += `\nTarget duration per episode: ${seconds} seconds`;
    return prompt;
  }
...
function getDefaultPromptBody(key) {
  switch (key) {
    ...
    case 'character_extraction':
      return '你是一个专业的角色分析师，擅长从剧本中提取和分析角色信息。\n\n**【语言要求】所有字段的值必须使用中文，禁止出现英文内容（role字段的值除外，固定为 main/supporting/minor）。**\n\n你的任务是根据提供的剧本内容，提取并整理剧中出现的所有有名字角色的设定。\n\n要求：\n1. 提取所有有名字的角色（忽略无名路人或背景角色）\n2. 对每个角色，提取以下信息（全部用中文填写）：\n   - name: 角色名字（中文）\n   - role: 角色类型，固定值之一：main / supporting / minor\n   - appearance: 外貌与身份事实（中文，100-200字，包含性别、年龄、体型、面部特征、发型、具体服装等，不含场景环境；不要写入画风名称、渲染方式或“符合/贴合某某风格”等评价句）\n   - description: 背景故事和角色关系（中文，50-100字）\n3. 主要角色外貌要详细，次要角色可以简化';
...
- 突出最具场景辨识度的标志性细节
- 强调光线、材质、氛围的统一性
- 悬疑/恐怖题材必须保留一个由剧情线索推导的**叙事异常点**；用中断的痕迹、违反常理的光线或错位物件制造不安，不得用蜘蛛网、血迹、雾气等廉价符号堆砌代替剧情证据
...
- 悬疑/恐怖题材必须让同一个由剧情线索推导的**叙事异常点**在四格中保持位置与形态一致；不得用蜘蛛网、血迹、雾气等廉价符号堆砌代替剧情证据
...
CONTEXT_NEXT: <next shot summary — ignore for image, relevant only for mood>`;
  ...
  // 中文版：身份特征与服装状态分层；服装只能来自连戏快照，不能从参考图或模型自由推断
...
5. **角色身份与服装分层铁律（最高优先级，任何违反均视为失败）**：
   - 角色外貌**仅允许使用固定身份特征**（脸型、五官、发型、肤质、皮肤纹理、独特标记、年龄性别等）。
   - **严禁自行添加或推断**任何服装；服装只能逐字来自 CURRENT_CONTINUITY_STATE 或 PREV_CONTINUITY_STATE。
   - 提供当前连戏状态时，必须把其中每个角色的 clothing 原样写入 prompt；角色参考图只锁脸、五官、发型、年龄和体型，参考图服装不得覆盖连戏状态。
   - 没有任何连戏状态时，才不在 prompt 中添加服装词；不得把参考图里的默认服装脑补进文字。
...
8. **服装与连戏一致性铁律**：
   - CURRENT_CONTINUITY_STATE 优先于 PREV_CONTINUITY_STATE；提供当前状态时必须**逐字匹配**该角色的 clothing。
   - 只有上一镜状态时，必须逐字继承上一镜头中该角色的 clothing。
   - 当前 ACTION 未明确写明“换衣服/脱外套/换装”等动作，则**绝不改变或重新描述服装**。
   - 没有任何连续状态时，完全不在 prompt 中出现任何服装相关词。
   - 连戏状态的服装优先级永远高于参考图中的默认服装。
...
CURRENT_CONTINUITY_STATE: <当前镜头经连戏校对后的状态快照 JSON；服装以此为最高优先级>
