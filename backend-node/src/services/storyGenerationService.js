...
async function generateStory(db, log, body) {
  ...
  const episodeDurationSeconds = Math.max(15, Math.round(Number(body.episode_duration_seconds) || 60));

  const systemPrompt = promptI18n.getStoryExpansionSystemPrompt(cfg, episodeCount, episodeDurationSeconds);
  const userPrompt = promptI18n.buildStoryExpansionUserPrompt(cfg, premise, style, type, episodeCount, episodeDurationSeconds);

  // 按成片时长给足结构化剧本空间，避免固定 800 字造成短片信息过载，也避免无谓 token 开销。
  const minTokensNeeded = Math.max(1400, episodeCount * Math.ceil(episodeDurationSeconds * 12));
